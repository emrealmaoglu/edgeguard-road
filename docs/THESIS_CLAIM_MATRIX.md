...
| Three final project-owned semantic models were trained, including one from scratch | `EGX-SEG-FINAL-{01..03}` | Cityscapes train roles; official val confirmation | Training curves and frozen semantic metrics | Three checkpoint bundles with init provenance | All runs complete; one `random_init`; human accepts comparison | `planned` |
