"""Resolve the exact successful Colab semantic runtime from its evidence receipt."""
...
_PATHS = {
    "hosted_current": ("runtime_current_root", "mmseg-path-a"),
    "isolated_py311": ("runtime_py311_root", "mmseg-path-b"),
}
...
def resolve_colab_runtime(
    ...
    *,
    ...
    """Validate and resolve interpreter/checkout without assuming fallback Path B."""
    payload = json.loads(receipt_path.read_text(encoding="utf-8"))
    if payload.get("record_type") != "semantic_compatibility_cascade_receipt":
        raise ValueError("invalid semantic compatibility receipt")
    if payload.get("project_commit") != expected_project_commit:
        raise ValueError("compatibility receipt belongs to another project commit")
    selected = str(payload.get("selected_path"))
    if selected not in _PATHS:
        raise ValueError("compatibility receipt has no supported selected path")
    runtime_key, checkout_name = _PATHS[selected]
    contract = payload.get("runtime_contract")
    environment = payload.get("environment")
    if not isinstance(contract, dict) or not isinstance(environment, dict):
        raise ValueError("compatibility receipt omits runtime or environment evidence")
    runtime_root = Path(str(contract[runtime_key]))
    ...
    probe = payload.get("five_model_probe")
    if not isinstance(probe, dict) or (
        probe.get("model_count") != 5 or probe.get("checkpoint_resume_verified") is not True
    ):
    ...
    return {
        ...
        "compatibility_receipt_sha256": sha256_file(receipt_path),
    }
